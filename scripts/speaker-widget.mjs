...
export class SpeakerWidget extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static DEFAULT_OPTIONS = {
    id: "live-actors-widget",
    window: { title: "Live Actors", resizable: false },
  ...
  static PARTS = {
    picker: { template: "modules/live-actors/templates/speaker-widget.hbs" },
  };
  ...
  static clearPin() {
    ...
    VideoAnimator.setGMPinnedToken(null);
    SpeakerWidget._instance?.render({ force: true });
  }
  ...
  static _onSelectToken(_event, target) {
    ...
    VideoAnimator.setGMPinnedToken(SpeakerWidget.pinnedTokenId);
    SpeakerWidget._instance?.render({ force: true });
  }
